...
Kubernetes (K8s) est un orchestrateur de conteneurs qui suit une architecture Control Plane / Workers.
**Le Control Plane**
Il prend les décisions pour le cluster et détecte/répond aux événements. Ses composants clés sont :
- **API Server** : le point d'entrée unique pour toutes les commandes (kubectl, Argo CD).
- **etcd** : la base de données du cluster qui stocke toute la configuration (état souhaité).
- **Scheduler** : il décide sur quel nœud Worker un nouveau Pod doit être lancé.
- **Controller Manager** : il veille à ce que l'état réel corresponde à l'état souhaité (ex: s'assurer qu'il y a bien 3 replicas).
...
- **CSI (Container Storage Interface)** : il permet de monter des Volumes Block Storage DigitalOcean sur les Pods PostgreSQL ou Gitea pour garantir que les données ne sont pas perdues si un Pod redémarre.
